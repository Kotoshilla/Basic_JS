...
// console.log(btns[1].classList.add('red')); //можно также через запятую добавлять классы 
...
// можно использовать и toggle

// console.log(btns[0].className); //Устаревший метод
//Лучше его не исползовать 

wrapper.addEventListener('click', (event) => {
    if (event.target && event.target.tagName == "BUTTON") {
        console.log('Hello');
    }
}); //Будет рабоать даже если добавлять новые элементы 
...
}); // На новых элементах не будет работать
